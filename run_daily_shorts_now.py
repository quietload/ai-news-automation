#!/usr/bin/env python3
"""
Daily Shorts - Immediate Upload (No Schedule)
=============================================
Generates and uploads daily news shorts immediately (public).
...
LOG_FILE = LOG_DIR / f"shorts_now_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def run_shorts():
    log("=" * 60)
    log("[1/2] Generating Shorts (6 news, max 60 seconds)...")
    log("=" * 60)
...
def main():
    log("\n")
    log("*" * 60)
    log("*  DAILY SHORTS - IMMEDIATE UPLOAD")
    log("*" * 60)
    ...
    try:
        ...
        log("\n" + "=" * 60)
        log("[2/2] Uploading Shorts to YouTube (PUBLIC)...")
        log("=" * 60)
